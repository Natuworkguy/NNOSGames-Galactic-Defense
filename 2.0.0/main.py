...
helptext = {
    ...
    "--nobgm":"Disable background music for game session.",
    "--debug":"Enable debug mode, which prints additional debug information (e.g., key presses) to the console."
}
if "--help" in sys.argv:
    pygame.quit()
    print("---NNOS Games: Galactic Defence---")
    print("Usage:")
    print("  [%s]: %s" %("--help", helptext["--help"]))
    print("  [%s]: %s" %("--nobgm", helptext["--nobgm"]))
    print("  [%s]: %s" %("--debug", helptext["--debug"]))
    exit()
def bgm():
    try:
        ...
        logging.error(f"Error: Image file '{filename}' not found in '{assets_dir}'")
try:
